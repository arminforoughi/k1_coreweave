...
Gemini Live API provides real-time audio conversation about the scene.

Usage:
    export GEMINI_API_KEY="your-key"
    python3 gemini_live_camera.py
    python3 gemini_live_camera.py --voice Charon --frame-interval 2.0
    python3 gemini_live_camera.py --confidence 0.4
"""
...
class CameraDetectionNode(Node):
    """ROS2 node: subscribes to left camera + stereo depth, runs YOLO detection."""
    ...
    def __init__(self, model_path='yolov8n.pt', confidence=0.5):
        ...
        self.latest_frame = None         # annotated frame (for web UI + Gemini)
        self.latest_detections = []      # detection dicts
        self._depth_map = None           # uint16 depth in mm
        self._raw_frame = None           # raw BGR frame
        self._fps = 0.0
        self._fps_counter = 0
        ...
        # Run detection at ~10 Hz (decoupled from camera rate)
    ...
    def _on_compressed(self, msg):
        """Fallback if raw topic isn't publishing."""
        if self._raw_frame is not None:
            return  # raw topic is active, skip compressed
        try:
    ...
    def _on_depth(self, msg):
        """Receive mono16 depth map (values in millimeters)."""
        try:
    ...
    def _get_depth_at(self, x, y, window=5):
        """Get depth in meters at pixel (x,y), median over small window."""
        ...
        if len(valid) == 0:
            return None
        # mono16 depth is in millimeters → convert to meters
        return float(np.median(valid)) / 1000.0

    def _detect_tick(self):
        """Timer callback: run YOLO on latest frame + overlay depth."""
        ...
        try:
            ...
            status = f"FPS: {self._fps:.0f} | Objects: {len(detections)} | {depth_str}"
            cv2.putText(
                annotated, status, (8, 24),
    ...
    def get_frame_b64jpeg(self, max_dim=640, quality=60):
        """Return latest annotated frame as base64 JPEG for Gemini."""
        with self._lock:
    ...
    def get_detection_summary(self):
        """Text summary of current detections."""
        with self._lock:
        ...
        for d in dets:
            ...
            lines.append(f"- {d['class']} ({d['confidence']:.0%}, {dist})")
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  #right { width:380px; display:flex; flex-direction:column; border-left:1px solid #333; }
  #header { padding:12px 16px; border-bottom:1px solid #333; font-size:14px; color:#888; }
  #header span { color:#4CAF50; font-weight:bold; }
  #detections { padding:8px 16px; border-bottom:1px solid #333; max-height:200px; overflow-y:auto; }
...
<body>
  ...
  <div id="right">
    <div id="header"><span>Gemini Live</span> &mdash; Detection + Depth + Voice</div>
    ...
    try {
      ...
      if (msgs.length !== lastLen) {
        ...
        chat.innerHTML = msgs.map(m =>
          `<div class="msg ${m.role === 'You' ? 'you' : 'robot'}">` +
          `<div class="role">${m.role}</div>${m.text}</div>`
        ).join('');
    ...
    try {
      ...
      if (dets.length === 0) {
        ...
        detList.innerHTML = dets.map(d => {
          ...
          return `<div class="det"><span><span class="cls">${d.class}</span> <span class="conf">${(d.confidence*100).toFixed(0)}%</span></span><span class="dist">${dist}</span></div>`;
        }).join('');
      }
    } catch(e) {}
...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/':
            ...
            data = json.dumps(get_transcript())
            self.send_response(200)
...
async def send_audio(session, pya):
    """Stream microphone PCM audio to Gemini Live."""
    stream = pya.open(
        format=FORMAT,
        channels=CHANNELS,
...
async def send_video(session, camera_node, interval):
    """Periodically send annotated camera frames to Gemini Live."""
    try:
        while True:
...
async def receive_responses(session, pya):
    """Receive and play audio responses; print transcriptions."""
    stream = pya.open(
        format=FORMAT,
        channels=CHANNELS,
...
async def run_session(api_key, camera_node, voice, frame_interval):
    ...
    config = types.LiveConnectConfig(
        ...
        system_instruction=(
            "You are a Booster K1 humanoid robot with stereo vision cameras. "
            "Video frames are streamed to you continuously with real-time object "
            "detection overlays — each detected object has a bounding box, class label, "
            "confidence score, and calibrated distance in meters from the stereo depth cameras. "
            "When someone asks what you see, describe the scene naturally using both "
            "the visual content and the detection/distance data shown on the frame. "
            "You can tell people how far away objects and people are. "
    ...
    try:
        async with client.aio.live.connect(
            ...
            tasks = [
                ...
                asyncio.create_task(
                    send_video(session, camera_node, frame_interval)
                ),
...
def main():
    parser = argparse.ArgumentParser(
        description='Gemini Live + Stereo Depth + YOLOv8 Object Detection'
    )
    parser.add_argument(
    ...
    print("=" * 60)
    print("Gemini Live + Stereo Depth + YOLOv8 Detection")
    print("=" * 60)

    # Start ROS2 + camera/detection node
    rclpy.init()
